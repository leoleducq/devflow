...
**The proxy.** When you need your project's *original* ports (say 3000 and 3005) to reach one particular environment — an OAuth callback, a webhook, muscle memory — run `devflow proxy` as a daemon and aim it with `devflow activate [env]` (`--none` to forward nowhere).
...
**Node.js 20+** and **git**, plus **Docker** for environment databases (LITE environments work without it). Optional: `lazysql` for `devflow db` (`--url` works without it), `psql` for querying a database by hand, [herdr](https://herdr.dev). Installing runs `prisma generate`, which builds a platform-specific query engine, so the first install needs network access. `devflow doctor` reports what is missing; `npx @iziatask/devflow doctor` tries it without installing.
